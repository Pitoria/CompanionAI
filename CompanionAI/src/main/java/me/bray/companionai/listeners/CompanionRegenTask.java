...
public class CompanionRegenTask {
    ...
    private void tick() {
        ...
        for (String uuid : plugin.getDataManager().getData().getConfigurationSection("players").getKeys(false)) {
            ...
            // CORREGIDO: Cambiado a LivingEntity para soportar cualquier tipo de entidad de NPC de manera segura
            if (!(npc.getEntity() instanceof LivingEntity npcEntity)) continue;

            // CORREGIDO: Obtener la vida máxima mediante atributos sin usar métodos deprecados
